...
rng default; rng(0);
Kmax = 10;
clust = zeros(size(X,1),Kmax);
for k=1:Kmax
    clust(:,k) = kmeans(X,k,'distance','sqeuclidean');
end
...
pscat = scatter(X(:,1),X(:,3),dotp,'o','MarkerEdgeColor',mg,'MarkerFaceColor',mg);
pscat.MarkerFaceAlpha = .2;
pscat.MarkerEdgeAlpha = .5;

title('Unlabeled Iris data');
